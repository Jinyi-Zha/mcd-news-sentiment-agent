# Email Delivery Preview

Recipient list placeholder: <recipient-list-placeholder>
Subject: MCD Equities Briefing | US Close | 2026-07-24 22:11 London
Source briefing path: outputs/us_close_briefing.md

## Email Body

MCD Capital — Equities Briefing | US Close | 2026-07-24 22:11 BST

EXECUTIVE SUMMARY
- Magnificent Seven / Big Tech / AI spending remains in focus: 6 sources / 15 headlines. Mega-cap tech breadth remains central to index direction.
- Microsoft / OpenAI / Azure remains in focus: 4 sources / 8 headlines. AI and cloud narratives keep mega-cap tech in focus.
- Nvidia / AI chip / Blackwell / semiconductor remains in focus: 3 sources / 4 headlines. Chip coverage could affect semiconductor leadership.

TOP MARKET THEMES
1. Theme: Magnificent Seven / Big Tech / AI spending
   Tickers: AAPL; AMZN; GOOGL; META; MSFT; NVDA; TSLA
   Evidence: 6 sources / 15 headlines
   Rep: Why Tesla, Google, and other Mag 7 stocks are losing billions in valuation
   Relevance: Mega-cap tech breadth remains central to index direction.
2. Theme: Microsoft / OpenAI / Azure
   Tickers: MSFT
   Evidence: 4 sources / 8 headlines
   Rep: OpenAI president Greg Brockman: 'We will remain in this compute shortage no matter what'
   Relevance: AI and cloud narratives keep mega-cap tech in focus.
3. Theme: Nvidia / AI chip / Blackwell / semiconductor
   Tickers: NVDA
   Evidence: 3 sources / 4 headlines
   Rep: AMD, Newly Emboldened, Is Taking Direct Aim at Nvidia
   Relevance: Chip coverage could affect semiconductor leadership.
4. Theme: Amazon / AWS
   Tickers: AMZN
   Evidence: 2 sources / 3 headlines
   Rep: This Dog Sniffs Out A Buy Point While Monitoring AI Behavior With AWS, Azure, Google
   Relevance: AWS headlines keep AI infrastructure spending in focus.

KEY TICKERS TO WATCH
1. GOOGL — Alphabet / Google / YouTube
   Evidence: 9 inferred mentions / Yahoo feed 8
   Why it matters: Search, YouTube and AI-platform coverage remain in focus.
   Watch next: search, YouTube and AI platform headlines.
2. MSFT — Microsoft / OpenAI / Azure
   Evidence: 8 inferred mentions / Yahoo feed 9
   Why it matters: AI capex and platform spending narratives remain in focus.
   Watch next: OpenAI, Azure and mega-cap tech leadership headlines.
3. TSLA — Tesla / Elon Musk / FSD
   Evidence: 7 inferred mentions / Yahoo feed 9
   Why it matters: Autonomy, deliveries and EV sentiment remain key swing factors.
   Watch next: FSD, robotaxi and delivery-related headlines.
4. NVDA — Nvidia / AI chip / Blackwell
   Evidence: 4 inferred mentions / Yahoo feed 9
   Why it matters: AI chips and data-centre demand remain central to semiconductor leadership.
   Watch next: chip demand, Blackwell and AI infrastructure headlines.

MACRO WATCH
No major macro events in the v1 manual calendar.
High-importance events may affect index futures, rates, FX, and sector leadership.

NEXT WATCH POINTS
- Top theme: watch whether Magnificent Seven / Big Tech / AI spending broadens or fades.
- Key tickers: watch follow-through in GOOGL, MSFT, TSLA.
- Macro: light calendar; equity themes may drive the session.

Generated automatically. Not a trade recommendation.
