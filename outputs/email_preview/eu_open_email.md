# Email Delivery Preview

Recipient list placeholder: <recipient-list-placeholder>
Subject: MCD Equities Briefing | EU Open | 2026-09-15 13:23 London
Source briefing path: outputs/eu_open_briefing.md

## Email Body

MCD Capital — Equities Briefing | EU Open | 2026-09-15 13:23 BST

EXECUTIVE SUMMARY
- Amazon / AWS remains in focus: 4 sources / 7 headlines. AWS headlines keep AI infrastructure spending in focus.
- Alphabet / Google / YouTube remains in focus: 3 sources / 6 headlines. Search, YouTube and AI coverage keep communication services in focus.
- Apple / iPhone / App Store remains in focus: 3 sources / 4 headlines. iPhone and App Store coverage keeps consumer tech in focus.

TOP MARKET THEMES
1. Theme: Amazon / AWS
   Tickers: AMZN
   Evidence: 4 sources / 7 headlines
   Rep: Amazon Could Buy Up to $60 Billion From Qualcomm Just as Apple Brings Modems In-House. Is the AI Pivot Real?
   Relevance: AWS headlines keep AI infrastructure spending in focus.
2. Theme: Alphabet / Google / YouTube
   Tickers: GOOGL
   Evidence: 3 sources / 6 headlines
   Rep: Alphabet May Have Just Gotten Its Biggest Competitor Yet. And It's Not OpenAI or Anthropic.
   Relevance: Search, YouTube and AI coverage keep communication services in focus.
3. Theme: Apple / iPhone / App Store
   Tickers: AAPL
   Evidence: 3 sources / 4 headlines
   Rep: Exclusive-Indian consumer regulator escalates probe into Apple's software warranty terms
   Relevance: iPhone and App Store coverage keeps consumer tech in focus.
4. Theme: Meta / Facebook / Instagram / WhatsApp
   Tickers: META
   Evidence: 3 sources / 4 headlines
   Rep: Fifty-Five Analysts Cover Meta and None Say Sell
   Relevance: Social platforms keep digital advertising exposure in focus.

KEY TICKERS TO WATCH
1. AMZN — Amazon / AWS
   Evidence: 7 inferred mentions / Yahoo feed 10
   Why it matters: AWS and cloud infrastructure spending remain key AI investment signals.
   Watch next: AWS, cloud capex and AI infrastructure headlines.
2. GOOGL — Alphabet / Google / YouTube
   Evidence: 6 inferred mentions / Yahoo feed 10
   Why it matters: Search, YouTube and AI-platform coverage remain in focus.
   Watch next: search, YouTube and AI platform headlines.
3. META — Meta / Facebook / Instagram / WhatsApp
   Evidence: 4 inferred mentions / Yahoo feed 8
   Why it matters: Digital ads, social platforms and AI infrastructure remain linked.
   Watch next: ad demand, platform engagement and AI spend headlines.
4. AAPL — Apple / iPhone / App Store
   Evidence: 4 inferred mentions / Yahoo feed 3
   Why it matters: Consumer tech, App Store and regulatory coverage remain active.
   Watch next: iPhone, App Store and policy headlines.

MACRO WATCH
No major macro events in the v1 manual calendar.
High-importance events may affect index futures, rates, FX, and sector leadership.

NEXT WATCH POINTS
- Top theme: watch whether Amazon / AWS broadens or fades.
- Key tickers: watch follow-through in AMZN, GOOGL, META.
- Macro: light calendar; equity themes may drive the session.

Generated automatically. Not a trade recommendation.
