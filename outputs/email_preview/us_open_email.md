# Email Delivery Preview

Recipient list placeholder: <recipient-list-placeholder>
Subject: MCD Equities Briefing | US Open | 2026-07-17 15:56 London
Source briefing path: outputs/us_open_briefing.md

## Email Body

MCD Capital — Equities Briefing | US Open | 2026-07-17 15:56 BST

EXECUTIVE SUMMARY
- Apple / iPhone / App Store remains in focus: 7 sources / 11 headlines. iPhone and App Store coverage keeps consumer tech in focus.
- Amazon / AWS remains in focus: 5 sources / 8 headlines. AWS headlines keep AI infrastructure spending in focus.
- Tesla / FSD / deliveries / robotaxi remains in focus: 5 sources / 6 headlines. Autonomy and deliveries keep high-beta EV sentiment in focus.

TOP MARKET THEMES
1. Theme: Apple / iPhone / App Store
   Tickers: AAPL
   Evidence: 7 sources / 11 headlines
   Rep: Apple Just Toppled Nvidia as World’s Most Valuable Stock. Here’s Why the Gap Will Widen
   Relevance: iPhone and App Store coverage keeps consumer tech in focus.
2. Theme: Amazon / AWS
   Tickers: AMZN
   Evidence: 5 sources / 8 headlines
   Rep: Meta Plans To Hire AWS Executive As It Explores Cloud Push: Report
   Relevance: AWS headlines keep AI infrastructure spending in focus.
3. Theme: Tesla / FSD / deliveries / robotaxi
   Tickers: TSLA
   Evidence: 5 sources / 6 headlines
   Rep: Mission aborted: Why Elon Musk canceled SpaceX's Starship launch
   Relevance: Autonomy and deliveries keep high-beta EV sentiment in focus.
4. Theme: Nvidia / AI chip / Blackwell / semiconductor
   Tickers: NVDA
   Evidence: 4 sources / 9 headlines
   Rep: Buffett’s Biggest Bet Just Dethroned Nvidia As the Largest Company In the World
   Relevance: Chip coverage could affect semiconductor leadership.

KEY TICKERS TO WATCH
1. AAPL — Apple / iPhone / App Store
   Evidence: 11 inferred mentions / Yahoo feed 10
   Why it matters: Consumer tech, App Store and regulatory coverage remain active.
   Watch next: iPhone, App Store and policy headlines.
2. NVDA — Nvidia / AI chip / Blackwell
   Evidence: 9 inferred mentions / Yahoo feed 10
   Why it matters: AI chips and data-centre demand remain central to semiconductor leadership.
   Watch next: chip demand, Blackwell and AI infrastructure headlines.
3. AMZN — Amazon / AWS
   Evidence: 8 inferred mentions / Yahoo feed 8
   Why it matters: AWS and cloud infrastructure spending remain key AI investment signals.
   Watch next: AWS, cloud capex and AI infrastructure headlines.
4. META — Meta / Facebook / Instagram / WhatsApp
   Evidence: 8 inferred mentions / Yahoo feed 8
   Why it matters: Digital ads, social platforms and AI infrastructure remain linked.
   Watch next: ad demand, platform engagement and AI spend headlines.

MACRO WATCH
No major macro events in the v1 manual calendar.
High-importance events may affect index futures, rates, FX, and sector leadership.

NEXT WATCH POINTS
- Top theme: watch whether Apple / iPhone / App Store broadens or fades.
- Key tickers: watch follow-through in AAPL, NVDA, AMZN.
- Macro: light calendar; equity themes may drive the session.

Generated automatically. Not a trade recommendation.
