# Email Delivery Preview

Recipient list placeholder: <recipient-list-placeholder>
Subject: MCD Equities Briefing | US Open | 2026-09-14 19:47 London
Source briefing path: outputs/us_open_briefing.md

## Email Body

MCD Capital — Equities Briefing | US Open | 2026-09-14 19:47 BST

EXECUTIVE SUMMARY
- Apple / iPhone / App Store remains in focus: 7 sources / 7 headlines. iPhone and App Store coverage keeps consumer tech in focus.
- Alphabet / Google / YouTube remains in focus: 5 sources / 13 headlines. Search, YouTube and AI coverage keep communication services in focus.
- Tesla / FSD / deliveries / robotaxi remains in focus: 5 sources / 8 headlines. Autonomy and deliveries keep high-beta EV sentiment in focus.

TOP MARKET THEMES
1. Theme: Apple / iPhone / App Store
   Tickers: AAPL
   Evidence: 7 sources / 7 headlines
   Rep: Apple's Siri Upgrade Does Not Require Every User to Buy Another iPhone
   Relevance: iPhone and App Store coverage keeps consumer tech in focus.
2. Theme: Alphabet / Google / YouTube
   Tickers: GOOGL
   Evidence: 5 sources / 13 headlines
   Rep: What Anthropic's AI Slowdown Call Could Mean For Cloud Giants Amazon, Microsoft, Google
   Relevance: Search, YouTube and AI coverage keep communication services in focus.
3. Theme: Tesla / FSD / deliveries / robotaxi
   Tickers: TSLA
   Evidence: 5 sources / 8 headlines
   Rep: Elon Musk: I've been 'sounding the alarm' on AI for a while
   Relevance: Autonomy and deliveries keep high-beta EV sentiment in focus.
4. Theme: Microsoft / OpenAI / Azure
   Tickers: MSFT
   Evidence: 4 sources / 9 headlines
   Rep: Apple Wants to Know What’s Hiding in OpenAI’s Secret Unreleased Device, and Who at the Company Had Access to It.
   Relevance: AI and cloud narratives keep mega-cap tech in focus.

KEY TICKERS TO WATCH
1. GOOGL — Alphabet / Google / YouTube
   Evidence: 13 inferred mentions / Yahoo feed 10
   Why it matters: Search, YouTube and AI-platform coverage remain in focus.
   Watch next: search, YouTube and AI platform headlines.
2. AMZN — Amazon / AWS
   Evidence: 11 inferred mentions / Yahoo feed 10
   Why it matters: AWS and cloud infrastructure spending remain key AI investment signals.
   Watch next: AWS, cloud capex and AI infrastructure headlines.
3. MSFT — Microsoft / OpenAI / Azure
   Evidence: 9 inferred mentions / Yahoo feed 10
   Why it matters: AI capex and platform spending narratives remain in focus.
   Watch next: OpenAI, Azure and mega-cap tech leadership headlines.
4. TSLA — Tesla / Elon Musk / FSD
   Evidence: 8 inferred mentions / Yahoo feed 10
   Why it matters: Autonomy, deliveries and EV sentiment remain key swing factors.
   Watch next: FSD, robotaxi and delivery-related headlines.

MACRO WATCH
No major macro events in the v1 manual calendar.
High-importance events may affect index futures, rates, FX, and sector leadership.

NEXT WATCH POINTS
- Top theme: watch whether Apple / iPhone / App Store broadens or fades.
- Key tickers: watch follow-through in GOOGL, AMZN, MSFT.
- Macro: light calendar; equity themes may drive the session.

Generated automatically. Not a trade recommendation.
