# Email Delivery Preview

Recipient list placeholder: <recipient-list-placeholder>
Subject: MCD Equities Briefing | US Open | 2026-07-16 16:17 London
Source briefing path: outputs/us_open_briefing.md

## Email Body

MCD Capital — Equities Briefing | US Open | 2026-07-16 16:17 BST

EXECUTIVE SUMMARY
- Tesla / FSD / deliveries / robotaxi remains in focus: 6 sources / 7 headlines. Autonomy and deliveries keep high-beta EV sentiment in focus.
- Nvidia / AI chip / Blackwell / semiconductor remains in focus: 4 sources / 6 headlines. Chip coverage could affect semiconductor leadership.
- Alphabet / Google / YouTube remains in focus: 4 sources / 4 headlines. Search, YouTube and AI coverage keep communication services in focus.

TOP MARKET THEMES
1. Theme: Tesla / FSD / deliveries / robotaxi
   Tickers: TSLA
   Evidence: 6 sources / 7 headlines
   Rep: Tesla and Rivian Are Both Down 12%. Here's the Better Buy for the Second Half of 2026.
   Relevance: Autonomy and deliveries keep high-beta EV sentiment in focus.
2. Theme: Nvidia / AI chip / Blackwell / semiconductor
   Tickers: NVDA
   Evidence: 4 sources / 6 headlines
   Rep: TSMC raises capex and revenue forecast, highlighting growing AI chip demand
   Relevance: Chip coverage could affect semiconductor leadership.
3. Theme: Alphabet / Google / YouTube
   Tickers: GOOGL
   Evidence: 4 sources / 4 headlines
   Rep: GOOGL, UBER, RDDT, XMTR Stocks Top Wedbush's Internet Picks – AI Called 'Single Most Impactful Swing Factor'
   Relevance: Search, YouTube and AI coverage keep communication services in focus.
4. Theme: IPO / M&A / deal
   Tickers: AMZN; GOOGL; META; MSFT; TSLA
   Evidence: 3 sources / 8 headlines
   Rep: This Data Center IPO Is the Next Big Test for the AI Trade
   Relevance: Deal flow may influence risk appetite and single-stock catalysts.

KEY TICKERS TO WATCH
1. TSLA — Tesla / Elon Musk / FSD
   Evidence: 7 inferred mentions / Yahoo feed 9
   Why it matters: Autonomy, deliveries and EV sentiment remain key swing factors.
   Watch next: FSD, robotaxi and delivery-related headlines.
2. NVDA — Nvidia / AI chip / Blackwell
   Evidence: 6 inferred mentions / Yahoo feed 8
   Why it matters: AI chips and data-centre demand remain central to semiconductor leadership.
   Watch next: chip demand, Blackwell and AI infrastructure headlines.
3. GOOGL — Alphabet / Google / YouTube
   Evidence: 4 inferred mentions / Yahoo feed 9
   Why it matters: Search, YouTube and AI-platform coverage remain in focus.
   Watch next: search, YouTube and AI platform headlines.
4. AAPL — Apple / iPhone / App Store
   Evidence: 4 inferred mentions / Yahoo feed 8
   Why it matters: Consumer tech, App Store and regulatory coverage remain active.
   Watch next: iPhone, App Store and policy headlines.

MACRO WATCH
No major macro events in the v1 manual calendar.
High-importance events may affect index futures, rates, FX, and sector leadership.

NEXT WATCH POINTS
- Top theme: watch whether Tesla / FSD / deliveries / robotaxi broadens or fades.
- Key tickers: watch follow-through in TSLA, NVDA, GOOGL.
- Macro: light calendar; equity themes may drive the session.

Generated automatically. Not a trade recommendation.
